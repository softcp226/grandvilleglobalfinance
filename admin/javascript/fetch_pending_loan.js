...
const create_element = (data) => {
  data.forEach((element) => {
    ...
    small.innerHTML = element.user.full_name || "unspecified";
    date_td.innerHTML = element.loan_duration || "Unspecified at the moment";
    plan.innerHTML = element.interest;
    amount_td.innerHTML = `$${element.loan_amount}`;
    idTxt.innerHTML = `${element.loan_type}`;
    img.src = `${element.payment_method_img || "css/1674653.png"}`;
    img.height = "17";
    img.hspace = "1";
    img.vspace = "1";
    payment_method_td.append(img);
    a.href = `/admin/loan-detail.html?${element._id}`;
    a.innerHTML = "Details";
...
(async () => {
  ...
  try {
    const response = await fetch(
      "https://invesco-global-backend.glitch.me/api/admin/loan_request",
